Shader "Nature/Terrain/SpecularWetArea" {
	...
	SubShader {
		...
		void surf(Input IN, inout SurfaceOutput o)
		{
			...
			half3 disturbMap = tex2D(_DisturbMap, IN.ref_Control *_DisturbTilling);

			half3 disturbNor =lerp(o.Normal,o.Normal * disturbMap, _DisturbMapFactor);
			//half3 disturbNor = disturbMap;
			...
			//refMask = tex2D(_RefMap,IN.ref_Control).r;

			//refMask = -IN.viewDirRim.y;
			//refMask = IN.viewDirRim.y;
			//o.Albedo += reflcol.rgb * refMask;
			o.Emission = reflcol.rgb * refMask;
			//o.Emission = fixed3(1, 0, 0);
			return;
   //         reflcol.rgb *= refMask;
   //         o.Emission = reflcol.rgb * 1;
			//return;
